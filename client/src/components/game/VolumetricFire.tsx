...
// Configure loaded fire texture
const configureFireTexture = (texture: THREE.Texture) => {
    ...
    super({
      ...
      fragmentShader: `
        ...
        vec4 samplerFire(vec3 p, vec4 scale) {
          ...
          // Mix with the educational fire class color
          vec3 fireColor = mix(texColor.rgb, color, 0.4);
          return vec4(fireColor, texColor.a);
        }
        ...
        void main() {
          ...
          for(int i = 0; i < ITERATIONS; i++) {
            ...
            lp.y += 0.5;
            lp.xz *= 2.0;
            col += samplerFire(lp, noiseScale) * intensity;
          }
...
const VolumetricFire: React.FC<VolumetricFireProps> = ({
  position,
  scale = 1,
  color = "#ff4500", 
  intensity = 1,
  ...
  const fireTexture = useLoader(THREE.TextureLoader, '/textures/fire.jpg');
  
  // Configure the loaded texture
  useMemo(() => {
    if (fireTexture) {
      configureFireTexture(fireTexture);
    }
  }, [fireTexture]);
  ...
  useLayoutEffect(() => {
    if (!ref.current || !fireTexture) return;
    
    const material = ref.current.material as FireMaterial;
    material.uniforms.fireTex.value = fireTexture;
    material.uniforms.color.value = new THREE.Color(color);
    material.uniforms.invModelMatrix.value = new THREE.Matrix4();
    material.uniforms.scale.value = new THREE.Vector3(scale, scale, scale);
    material.uniforms.intensity.value = intensity;
    material.uniforms.seed.value = Math.random() * 19.19;
  }, [color, scale, intensity, fireTexture]);
