...
import React, { useRef, useMemo, useEffect, Suspense, forwardRef, useState, useCallback, useImperativeHandle } from "react";
...
const useFollowCam = function({
  ...
  const originZDis = useRef(camInitDis != null ? camInitDis : -5);
...
const Ecctrl = ({
  ...
  const initialGravityScale = useMemo(() => {
    var _a2;
    return (_a2 = props.gravityScale) != null ? _a2 : 1;
  }, []);
  let massRatio = 1;
  ...
  const pivotXAxis = useMemo(() => new THREE.Vector3(1, 0, 0), []);
  const pivotYAxis = useMemo(() => new THREE.Vector3(0, 1, 0), []);
  const pivotZAxis = useMemo(() => new THREE.Vector3(0, 0, 1), []);
  ...
  const slopeRayOriginRef = useRef(null);
  ...
  const bodySensorRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (followLight && characterModelRef.current) {
      dirLight = (_b2 = (_a2 = characterModelRef.current.parent) == null ? void 0 : _a2.parent) == null ? void 0 : _b2.children.find(
        (item) => {
  ...
  useEffect(() => {
    var _a2;
    (_a2 = characterRef.current) == null ? void 0 : _a2.setEnabledRotations(
  ...
  useFrame((state, delta) => {
    ...
    pivotPosition.copy(currentPos).addScaledVector(pivotXAxis, camTargetPos.x).addScaledVector(pivotYAxis, camTargetPos.y + (capsuleHalfHeight + capsuleRadius / 2)).addScaledVector(pivotZAxis, camTargetPos.z);
    ...
    if ((jump || button1Pressed) && canJump) {
      ...
      (_a2 = rayHit == null ? void 0 : rayHit.collider.parent()) == null ? void 0 : _a2.applyImpulseAtPoint(characterMassForce, standingForcePoint, true);
    }
    ...
    rayHit = world.castRay(
      ...
      (collider) => {
        ...
        return !(data == null ? void 0 : data.excludeEcctrlRay);
      }
    );
    ...
    if (rayHit && canJump) {
      if (rayHit.collider.parent()) {
        ...
        const rayHitObjectBodyType = (_b2 = rayHit.collider.parent()) == null ? void 0 : _b2.bodyType();
        const rayHitObjectBodyMass = (_c2 = rayHit.collider.parent()) == null ? void 0 : _c2.mass();
        ...
        if (rayHitObjectBodyType === 0 || rayHitObjectBodyType === 2) {
          ...
          distanceFromCharacterToObject.copy(currentPos).sub((_d2 = rayHit.collider.parent()) == null ? void 0 : _d2.translation());
          const movingObjectLinvel = (_e2 = rayHit.collider.parent()) == null ? void 0 : _e2.linvel();
          const movingObjectAngvel = (_f2 = rayHit.collider.parent()) == null ? void 0 : _f2.angvel();
          ...
          if (rayHitObjectBodyType === 0) {
            ...
            (_g2 = rayHit.collider.parent()) == null ? void 0 : _g2.applyImpulseAtPoint(
    ...
    (_h2 = slopeRayOriginRef.current) == null ? void 0 : _h2.getWorldPosition(slopeRayorigin);
    ...
    if (slopeRayHit) {
      actualSlopeNormal = (_j2 = (_i2 = slopeRayHit.collider.castRayAndGetNormal(
        slopeRayCast,
        slopeRayLength,
        false
      )) == null ? void 0 : _i2.normal) != null ? _j2 : null;
    ...
    if (rayHit != null) {
      if (canJump && rayHit.collider.parent()) {
        ...
        (_k2 = rayHit.collider.parent()) == null ? void 0 : _k2.applyImpulseAtPoint(characterMassForce, standingForcePoint, true);
      }
    }
    if (!forward && !backward && !leftward && !rightward && canJump && joystickDis === 0 && !isPointMoving && !gamepadKeys.forward && !gamepadKeys.backward && !gamepadKeys.leftward && !gamepadKeys.rightward) {
      if (!isOnMovingObject) {
