...
const Ecctrl = ({
  ...
  let canJump = false;
  let isFalling = false;
  const initialGravityScale = React.useMemo(() => {
    var _a2;
  ...
  const pivotXAxis = React.useMemo(() => new THREE__namespace.Vector3(1, 0, 0), []);
  const pivotYAxis = React.useMemo(() => new THREE__namespace.Vector3(0, 1, 0), []);
  const pivotZAxis = React.useMemo(() => new THREE__namespace.Vector3(0, 0, 1), []);
  ...
  const slopeRayOriginRef = React.useRef(null);
  ...
  const bodySensorRef = React.useRef(null);
  ...
  fiber.useFrame((state, delta) => {
    ...
    rayHit = world.castRay(
      ...
      rayLength,
      false,
      rapier3dCompat.QueryFilterFlags.EXCLUDE_SENSORS,
      void 0,
      void 0,
      characterRef.current,
